...
public class Assembler {
   ...
   //Finds where all the labels are referenced
    ...
    private String jmplt(String firstArg, String secondArg, int line) { //change "JMPLE" to "JMPLT"
        ...
        } else if (firstArg.toUpperCase().contains("RO")){
            errorList.add("Error: Missing less than sign for JMPLT on line " + line);
        } else {                                        //change "JMPLE" to "JMPLT"
            errorList.add("Error: Invalid comparision register, must be R0, on " + line);
        }
        return result;
    }
    ...
    private String jump(String firstArg, int line) {
        ...
        if (labelMap.containsKey(firstArg)) { // arg is a label
            ...
            errorList.add("Error: Invalid destination for JMP on line " + line);
